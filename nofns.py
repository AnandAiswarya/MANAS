...
def region_of_interest(image):
    ...
    polygons = np.array([
        [(200, height), (1100, height), (550, 400)]
    ])
...
while(cap.isOpened()):
    ...
    final_mask =cv2.bitwise_and(frame, frame, mask=combo_mask)
    cropped_image = region_of_interest(final_mask)
    canny_image = canny_edge_detector(cropped_image)
    lines = cv2.HoughLinesP(canny_image, rho=2, theta=np.pi / 180,
